...
class CommentRepository {
    ...
    getComment(mysqlInsert){
        let mySql = `SELECT users.userId, users.firstname, users.surname, comment.userId, comment.content, comment.createdAt FROM users INNER JOIN comment ON users.userId = comment.userId WHERE comment.postId = userId ORDER BY comment.createdAt DESC`;
        mySql = mysql.format(mySql, mysqlInsert);
        return new Promise((resolve) => {
            db.query(mySql, (error, result, fields) => {
                if (error) throw error;
    ...
    deleteComment(mysqlInsert1, mysqlInsert2) {
        let mySql = `SELECT * FROM comment WHERE commentId = ?`;
